...
@section('content')
    <div class="container mx-auto p-8 bg-white rounded-lg shadow-xl border border-gray-200">
        <h1 class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 via-red-500 to-pink-500 mb-8">Tambah Stok</h1>
        ...
        <form action="{{ route('stok.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="number" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" id="jml_keluar" name="jml_keluar" required>
            </div>
            <div>
                <label for="total_barang" class="block text-gray-700 font-semibold">Total Barang</label>
                <input type="number" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" id="total_barang" name="total_barang" required>
            </div>
